...
The MJCF must already be generated by `tools/asset/usd_to_mjcf.py`. The script:
...
Example:
    # bare run — auto-picks the newest model_*.pt under logs/rsl_rl
...
def find_latest_checkpoint() -> Path | None:
    """Pick the newest model_*.pt under logs/rsl_rl (by run mtime, then iter)."""
...
# Must match Mos20262ClosedUsdEnvCfg.init_state.joint_pos (env_cfg.py). The
# rear/front hips were widened 0.06 -> 0.15 to spread the stance; thighs and
# shanks rest at 0. The env's _capture_usd_default_joint_state seeds
# default_joint_pos from exactly these values for the 12 actuated joints.
DEFAULT_JOINT_POS = np.array([
    0.15, 0.15, -0.15, -0.15,  # hips (fl, fr, rl, rr)
    0.0, 0.0, 0.0, 0.0,        # thighs
    0.0, 0.0, 0.0, 0.0,        # shank links
])
# Per-joint action scale — matches env_cfg.action_scale exactly. Using a flat
# 0.5 (the old value) compresses thigh/shank motion to 0.5/0.8145 = 61% of the
# trained range, so the robot can't lift its legs enough to walk.
ACTION_SCALE = np.array([
    0.5, 0.5, 0.5, 0.5,            # hips
    0.8145, 0.8145, 0.8145, 0.8145,  # thighs
    0.8145, 0.8145, 0.8145, 0.8145,  # shank links
])
ACTION_CLIP = 1.5
INIT_HEIGHT = 0.35
...
CONTROL_HZ = 50
# Training ran 200 Hz physics, but Isaac's closed-loop legs are rigid PhysX
# reduced-coordinate joints. MuJoCo models them as soft <connect> equalities,
# which need a small timestep to stay rigid yet stable (constraint timeconst
# must be >= 2*dt). 1000 Hz lets the loop be stiff enough that the lower leg
# bears weight instead of flopping; the 50 Hz policy control is unchanged.
SIM_HZ = 1000
...
def load_actor(checkpoint_path: Path, obs_dim: int, action_dim: int) -> ActorMLP:
    ...
    # Two on-disk formats exist in logs/:
    #   old (pre-rename rsl_rl): {"actor_state_dict": {"mlp.0.weight", ...}}
    #   new (env_isaaclab rsl_rl): {"model_state_dict": {"actor.0.weight", ...}}
    # Normalize both to the local ActorMLP's "mlp.*" keys.
    ...
    # Derive hidden sizes from the weights instead of hardcoding (256, 256, 128),
    # so an architecture change fails loudly at shape level, never silently.
    weight_keys = sorted(
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--checkpoint", default=None,
                    help=".pt file from rsl_rl (must contain actor_state_dict). "
                         "Defaults to the newest model_*.pt under logs/rsl_rl.")
