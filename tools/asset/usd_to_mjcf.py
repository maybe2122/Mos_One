...
Strategy:
  ...
  - Collision geom per body is a single box derived from the AABB of the
    ...
    the body's high-poly STL visual mesh is also exported at full resolution
    (no decimation) to deploy/mujoco/assets/meshes/<body>.obj and emitted
...
Run:
    /home/sz/code/rl/env_isaaclab/bin/python3 tools/asset/usd_to_mjcf.py
"""
...
# Fraction of triangles to keep when decimating the high-poly STL visual
# meshes (quadric edge collapse via trimesh + fast_simplification). 1.0 means
# no decimation — the full robot is only ~840k triangles, which MuJoCo loads
# in well under a second, and every decimation level tested chews up the small
# screw holes / bolt circles into jagged tooth-like artifacts (the old 4 mm
# grid vertex clustering was far worse). Purely visual — collision/physics
# are unchanged.
MESH_KEEP_RATIO = 1.0
# Below this face count there is nothing worth decimating — export as-is.
MESH_MIN_FACES = 4000
...
def decimate_mesh(points: np.ndarray, tris: np.ndarray, keep_ratio: float) -> tuple[np.ndarray, np.ndarray]:
    """Weld duplicate vertices, then optionally quadric edge-collapse decimate.

    The USD visual meshes carry STL-style per-facet duplicated vertices;
    `process=True` welds them, which shrinks the OBJ ~3x and lets quadric
    collapse see the surface as connected."""
    import trimesh

    mesh = trimesh.Trimesh(vertices=points, faces=tris, process=True)
    if keep_ratio < 1.0 and len(mesh.faces) > MESH_MIN_FACES:
        target = max(MESH_MIN_FACES, int(len(mesh.faces) * keep_ratio))
...
def export_visual_meshes(base_stage: Usd.Stage, body_names: list[str]) -> set[str]:
    ...
    for name in body_names:
        ...
        if loaded is None:
            continue
        pts, tris = loaded
        dpts, dtris = decimate_mesh(pts, tris, MESH_KEEP_RATIO)
        if len(dtris) == 0:
            continue
    ...
    print(f"[info] exported {len(written)} meshes, {total_in} -> {total_out} tris "
          f"(verts welded, quadric keep {MESH_KEEP_RATIO:.0%})")
    return written
